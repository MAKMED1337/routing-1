...
std::vector<uint16_t> build_partition(const Graph &graph, uint32_t regions, PartitionMethod method) {
    if (regions == 0 || regions > 65535) {
        throw std::invalid_argument("regions must be in [1, 65535]");
    }
    if (method == PartitionMethod::Grid) {
        return partition_grid(graph, regions);
    }
    if (method == PartitionMethod::Inertial) {
        return partition_inertial(graph, regions);
    }
    throw std::invalid_argument("unknown partition method enum value");
}

std::vector<uint16_t> build_partition(const Graph &graph, uint32_t regions, std::string_view method) {
    return build_partition(graph, regions, parse_partition_method(method));
}
