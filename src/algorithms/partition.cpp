...
std::vector<uint16_t> partition_grid(const Graph &graph, uint16_t regions) {
    ...
    for (VertexId v = 0; v < V; ++v) {
        const uint32_t r =
            lat_range > 0.0
                ? std::min(static_cast<uint32_t>((graph.coords[v].lat - lat_min) / lat_range * rows), rows - 1)
                : 0u;
        const uint32_t c =
            lon_range > 0.0
                ? std::min(static_cast<uint32_t>((graph.coords[v].lon - lon_min) / lon_range * cols), cols - 1)
                : 0u;
...
std::vector<uint16_t> partition_inertial(const Graph &graph, uint16_t regions) {
    ...
    kd_split(graph, all, result, 0, static_cast<uint32_t>(regions), 0);
    return result;
}
