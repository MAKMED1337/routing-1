...
- Algorithms must not construct expensive shared preprocessing artifacts (CH, PHAST) internally, except the algorithm whose purpose is building/querying that artifact. Inject prebuilt artifacts via constructors or an explicit shared context (e.g. `RoutingPreprocessingContext`); do not add silent internal fallback construction. Benchmarks must report dependency preprocessing (CH/PHAST/context) and algorithm-specific preprocessing as separate timings.
